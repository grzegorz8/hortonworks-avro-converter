...
public class AdditionalAvroDataTest {
  ...
  @Before
  public void before() {
    AvroDataConfig avroDataConfig = AvroDataConfig.AvroDataConfigBuilder.anAvroDataConfig()
        .withSchemasCacheSize(1)
        .withConnectMetaData(false)
        .withEnhancedAvroSchemaSupport(true)
        .build();
  ...
  public void testDocumentationPreservedSchema() throws IOException {
    ...
    assertEquals(avroSchema, outputAvroSchema);
  }

  @Test
  public void testDocumentationPreservedData() {
    ...
    assertEquals(SpecificData.get().toString(testRecord), SpecificData.get().toString(output));
  }
  ...
  @Test
  public void testComplexUnionData() {
    PodamFactory factory = new PodamFactoryImpl();
    ...
    assertEquals(SpecificData.get().toString(avroMessage), SpecificData.get().toString(output));
  }


  @Test
  public void testComplexMultiUnionData() {
    PodamFactory factory = new PodamFactoryImpl();
  ...
  /**
   * @see <a href="https://github.com/confluentinc/schema-registry/issues/405">ISSUE 405</a>.
   */
  @Test
  public void testNestedUnion() {
    ...
    Schema myAvroObjectSchema = new Parser().parse(
        "{"
            ...
            + "  \"namespace\" : \"io.sqooba.avro.AdditionalAvroDataTest$\","
            + "  \"fields\" : [ {"
            + "    \"name\" : \"obj\","
            + "    \"type\" : [ \"null\", {"
            + "      \"type\" : \"record\","
