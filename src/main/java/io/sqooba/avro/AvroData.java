...
public class AvroData {
  ...
  public AvroData(int cacheSize) {
    this(AvroDataConfig.AvroDataConfigBuilder.anAvroDataConfig()
        .withSchemasCacheSize(cacheSize)
        .build());
  }

  public AvroData(AvroDataConfig avroDataConfig) {
    fromConnectSchemaCache =
        new SynchronizedCache<>(new LRUCache<>(avroDataConfig.getSchemasCacheSize()));
